...
describe("Low-End Mobile Optimization", () => {
  ...
  describe("Mobile Arena Bounds", () => {
    it("should calculate arena for iPhone SE (375x667)", () => {
      ...
      // Should have reasonable dimensions for 4:3 aspect ratio
  ...
  describe("Performance Settings", () => {
    ...
    it("should scale particle count for performance", () => {
      ...
      expect(lowSettings.maxParticles).toBeLessThan(
        mediumSettings.maxParticles,
      );
      expect(mediumSettings.maxParticles).toBeLessThan(
        highSettings.maxParticles,
      );
    });
  ...
  describe("WCAG AA Touch Target Compliance", () => {
    it("should meet minimum 44px touch target requirement", () => {
      ...
      expect(extraSmallButtons.buttonHeight).toBeGreaterThanOrEqual(
        MINIMUM_TOUCH_TARGET,
      );

      const mobileButtons = getCombatLayoutConstants(400);
      expect(mobileButtons.buttonHeight).toBeGreaterThanOrEqual(
        MINIMUM_TOUCH_TARGET,
      );
    });
