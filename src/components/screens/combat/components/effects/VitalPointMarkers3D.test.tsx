...
describe("VitalPointMarkers3D", () => {
  ...
  describe("Region Filtering", () => {
    it("should filter head region correctly", () => {
      const headPoints = KOREAN_VITAL_POINTS.filter((vp) =>
        vp.id.startsWith("head_"),
      );
      expect(headPoints.length).toBe(12);
    });

    it("should filter torso region correctly", () => {
      const torsoPoints = KOREAN_VITAL_POINTS.filter((vp) =>
        vp.id.startsWith("torso_"),
      );
  ...
  describe("Severity Filtering", () => {
    ...
    it("should handle multiple severity filters", () => {
      const multiplePoints = KOREAN_VITAL_POINTS.filter((vp) =>
        [VitalPointSeverity.LETHAL, VitalPointSeverity.CRITICAL].includes(
          vp.severity,
        ),
      );
    ...
    it("should have all 70 points distributed across severity levels", () => {
      ...
      const totalPoints = allSeverities.reduce((sum, severity) => {
        const count = KOREAN_VITAL_POINTS.filter(
          (vp) => vp.severity === severity,
        ).length;
        return sum + count;
      }, 0);
  ...
  describe("Search Filtering", () => {
    it("should filter by Korean name when present in data", () => {
      ...
      const query = "경추";
      const matchingPoints = KOREAN_VITAL_POINTS.filter((vp) =>
        vp.names.korean.toLowerCase().includes(query.toLowerCase()),
      );
    ...
    it("should filter by English name", () => {
      const query = "temple";
      const matchingPoints = KOREAN_VITAL_POINTS.filter((vp) =>
        vp.names.english.toLowerCase().includes(query.toLowerCase()),
      );
      expect(matchingPoints).toBeDefined();
      expect(Array.isArray(matchingPoints)).toBe(true);
    });
    ...
    it("should filter by romanized name when present", () => {
      ...
      const query = "yeop";
      const matchingPoints = KOREAN_VITAL_POINTS.filter((vp) =>
        vp.names.romanized.toLowerCase().includes(query.toLowerCase()),
      );
      expect(matchingPoints).toBeDefined();
      expect(Array.isArray(matchingPoints)).toBe(true);
    });
    ...
    it("should filter by vital point ID", () => {
      const query = "head_";
      const matchingPoints = KOREAN_VITAL_POINTS.filter((vp) =>
        vp.id.toLowerCase().includes(query.toLowerCase()),
      );
    ...
    it("should be case-insensitive", () => {
      ...
      const lowerMatches = KOREAN_VITAL_POINTS.filter((vp) =>
        vp.names.english.toLowerCase().includes(lowerQuery.toLowerCase()),
      );
      const upperMatches = KOREAN_VITAL_POINTS.filter((vp) =>
        vp.names.english.toLowerCase().includes(upperQuery.toLowerCase()),
      );
    ...
    it("should handle empty search query", () => {
      const query: string = "";
      // Empty query matches all points
      const matchingPoints =
        query === ""
          ? KOREAN_VITAL_POINTS
          : KOREAN_VITAL_POINTS.filter((vp) =>
              vp.names.english.toLowerCase().includes(query.toLowerCase()),
            );
  ...
  describe("Combined Filtering", () => {
    ...
    it("should apply all three filters together", () => {
      ...
      const searchedPoints = lethalHeadPoints.filter((vp) =>
        vp.names.english.toLowerCase().includes("tem"),
      );
    ...
    it("should return empty array when no matches", () => {
      const impossibleSearch = KOREAN_VITAL_POINTS.filter((vp) =>
        vp.names.english.toLowerCase().includes("nonexistentpoint12345"),
      );
  ...
  describe("BodyRegionFilter Type", () => {
    it("should accept all valid region filter values", () => {
      const validFilters: BodyRegionFilter[] = [
        "all",
        "head",
        "torso",
        "arms",
