/**
 * Combat Components Index
 *
 * Exports all combat-related components for cleaner imports.
 */

// 3D Components
export { BloodDecals3D } from "./BloodDecals3D";
export { BloodParticles3D } from "./BloodParticles3D";
export { CombatArena3D } from "./CombatArena3D";
export { HitEffects3D } from "./HitEffects3D";
export { TraumaOverlay3D } from "./TraumaOverlay3D";
export { VitalPointMarkers3D } from "./VitalPointMarkers3D";
// Vital Point overlay controls
export { VitalPointOverlayControls } from "./VitalPointOverlayControls";
export type { BodyRegionFilter } from "./VitalPointOverlayControls";

// HUD Components
export { ActionFeedback } from "./ActionFeedback";
export { BodyPartHealthDisplay } from "./BodyPartHealthDisplay";
export { CombatControlsPanel } from "./CombatControlsPanel";
export { CombatReadinessBar } from "./CombatReadinessBar";
export { CombatTimer } from "./CombatTimer";
export { ComboCounter } from "./ComboCounter";
export { DamageNumbers } from "./DamageNumbers";
export { FPSMonitor } from "./FPSMonitor";
export { HealthBar } from "./HealthBar";
export { MatchCountdown } from "./MatchCountdown";
export { MobileControlsWrapper } from "./MobileControlsWrapper";
export { PauseMenu } from "./PauseMenu";
export { ControlsGuide } from "./ControlsGuide";
export { PlayerHUD } from "./PlayerHUD";
export { RoundAnnouncement } from "./RoundAnnouncement";
export { RoundStartAnnouncement } from "./RoundStartAnnouncement";
export { StaminaBar } from "./StaminaBar";
export { TechniqueBar } from "./TechniqueBar";
export { TechniqueCard } from "./TechniqueCard";
export { QuickSettings } from "./QuickSettings";

// UI Components
export { ConfirmDialog } from "../../ui/shared/ConfirmDialog";

// Types
export type { ActionFeedbackProps } from "./ActionFeedback";
export type { BodyPartHealthDisplayProps } from "./BodyPartHealthDisplay";
export type { CombatControlsPanelProps } from "./CombatControlsPanel";
export type { CombatReadinessBarProps } from "./CombatReadinessBar";
export type { CombatTimerProps } from "./CombatTimer";
export type { ComboCounterProps } from "./ComboCounter";
export type { DamageNumbersProps } from "./DamageNumbers";
export type { FPSMonitorProps } from "./FPSMonitor";
export type { HealthBarProps } from "./HealthBar";
export type { MatchCountdownProps } from "./MatchCountdown";
export type { MobileControlsWrapperProps } from "./MobileControlsWrapper";
export type { PauseMenuProps } from "./PauseMenu";
export type { PlayerHUDProps } from "./PlayerHUD";
export type { RoundAnnouncementProps } from "./RoundAnnouncement";
export type { RoundStartAnnouncementProps } from "./RoundStartAnnouncement";
export type { StaminaBarProps } from "./StaminaBar";
export type { TechniqueBarProps } from "./TechniqueBar";
export type { TechniqueCardProps } from "./TechniqueCard";
