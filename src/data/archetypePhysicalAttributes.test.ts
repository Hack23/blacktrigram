...
describe("Archetype Physical Attributes Data", () => {
  describe("무사 (Musa) Physical Profile", () => {
    it("should have balanced warrior attributes", () => {
      expect(MUSA_PHYSICAL.weight).toBe(75);
      expect(MUSA_PHYSICAL.legLength).toBe(95);
      expect(MUSA_PHYSICAL.armLength).toBe(75);
      expect(MUSA_PHYSICAL.muscleMass).toBe(38);
      expect(MUSA_PHYSICAL.fatMass).toBe(12);
      expect(MUSA_PHYSICAL.age).toBe(32);
    });

    it("should have high muscle mass for power", () => {
      expect(MUSA_PHYSICAL.muscleMass).toBeGreaterThan(35);
    });
  ...
  describe("암살자 (Amsalja) Physical Profile", () => {
    it("should have lean assassin attributes", () => {
      expect(AMSALJA_PHYSICAL.weight).toBe(68);
      expect(AMSALJA_PHYSICAL.legLength).toBe(98);
      expect(AMSALJA_PHYSICAL.armLength).toBe(78);
      expect(AMSALJA_PHYSICAL.muscleMass).toBe(32);
      expect(AMSALJA_PHYSICAL.fatMass).toBe(9);
    ...
    it("should have longest reach for precision", () => {
      expect(AMSALJA_PHYSICAL.legLength).toBeGreaterThan(MUSA_PHYSICAL.legLength);
      expect(AMSALJA_PHYSICAL.armLength).toBeGreaterThan(MUSA_PHYSICAL.armLength);
    });
  ...
  describe("해커 (Hacker) Physical Profile", () => {
    it("should have average cyber warrior attributes", () => {
      expect(HACKER_PHYSICAL.weight).toBe(70);
      expect(HACKER_PHYSICAL.legLength).toBe(92);
      expect(HACKER_PHYSICAL.armLength).toBe(73);
      expect(HACKER_PHYSICAL.muscleMass).toBe(34);
      expect(HACKER_PHYSICAL.fatMass).toBe(14);
      expect(HACKER_PHYSICAL.age).toBe(26);
    });

    it("should have average physical attributes", () => {
      const avgWeight = (MUSA_PHYSICAL.weight + AMSALJA_PHYSICAL.weight + JOJIK_PHYSICAL.weight) / 3;
      expect(HACKER_PHYSICAL.weight).toBeGreaterThan(avgWeight - 10);
      expect(HACKER_PHYSICAL.weight).toBeLessThan(avgWeight + 10);
    });
  ...
  describe("정보요원 (Jeongbo) Physical Profile", () => {
    it("should have intelligence operative attributes", () => {
      expect(JEONGBO_PHYSICAL.weight).toBe(73);
      expect(JEONGBO_PHYSICAL.legLength).toBe(94);
      expect(JEONGBO_PHYSICAL.armLength).toBe(74);
      expect(JEONGBO_PHYSICAL.muscleMass).toBe(36);
      expect(JEONGBO_PHYSICAL.fatMass).toBe(11);
  ...
  describe("조직폭력배 (Jojik) Physical Profile", () => {
    it("should have heavy street fighter attributes", () => {
      expect(JOJIK_PHYSICAL.weight).toBe(85);
      expect(JOJIK_PHYSICAL.legLength).toBe(90);
      expect(JOJIK_PHYSICAL.armLength).toBe(76);
      expect(JOJIK_PHYSICAL.muscleMass).toBe(42);
      expect(JOJIK_PHYSICAL.fatMass).toBe(18);
    ...
    it("should have highest muscle mass", () => {
      expect(JOJIK_PHYSICAL.muscleMass).toBeGreaterThan(MUSA_PHYSICAL.muscleMass);
      expect(JOJIK_PHYSICAL.muscleMass).toBeGreaterThan(AMSALJA_PHYSICAL.muscleMass);
    });
  ...
  describe("Archetype Physical Attributes Map", () => {
    it("should contain all five archetypes", () => {
      ...
      expect(ARCHETYPE_PHYSICAL_ATTRIBUTES[PlayerArchetype.AMSALJA]).toBeDefined();
      expect(ARCHETYPE_PHYSICAL_ATTRIBUTES[PlayerArchetype.HACKER]).toBeDefined();
      expect(ARCHETYPE_PHYSICAL_ATTRIBUTES[PlayerArchetype.JEONGBO_YOWON]).toBeDefined();
      expect(ARCHETYPE_PHYSICAL_ATTRIBUTES[PlayerArchetype.JOJIK_POKRYEOKBAE]).toBeDefined();
    });

    it("should map to correct physical profiles", () => {
      expect(ARCHETYPE_PHYSICAL_ATTRIBUTES[PlayerArchetype.MUSA]).toEqual(MUSA_PHYSICAL);
      expect(ARCHETYPE_PHYSICAL_ATTRIBUTES[PlayerArchetype.AMSALJA]).toEqual(AMSALJA_PHYSICAL);
      expect(ARCHETYPE_PHYSICAL_ATTRIBUTES[PlayerArchetype.HACKER]).toEqual(HACKER_PHYSICAL);
      expect(ARCHETYPE_PHYSICAL_ATTRIBUTES[PlayerArchetype.JEONGBO_YOWON]).toEqual(JEONGBO_PHYSICAL);
      expect(ARCHETYPE_PHYSICAL_ATTRIBUTES[PlayerArchetype.JOJIK_POKRYEOKBAE]).toEqual(JOJIK_PHYSICAL);
    });
  });

  describe("getArchetypePhysicalAttributes()", () => {
    it("should return correct attributes for each archetype", () => {
      expect(getArchetypePhysicalAttributes(PlayerArchetype.MUSA)).toEqual(MUSA_PHYSICAL);
      expect(getArchetypePhysicalAttributes(PlayerArchetype.AMSALJA)).toEqual(AMSALJA_PHYSICAL);
      expect(getArchetypePhysicalAttributes(PlayerArchetype.HACKER)).toEqual(HACKER_PHYSICAL);
      expect(getArchetypePhysicalAttributes(PlayerArchetype.JEONGBO_YOWON)).toEqual(JEONGBO_PHYSICAL);
      expect(getArchetypePhysicalAttributes(PlayerArchetype.JOJIK_POKRYEOKBAE)).toEqual(JOJIK_PHYSICAL);
    });
...
describe("Combat Calculation Functions", () => {
  describe("calculateEffectiveReach()", () => {
    ...
    it("should reflect archetype reach differences", () => {
      const musaKickReach = calculateEffectiveReach(MUSA_PHYSICAL.legLength, 1.0);
      const amsaljaKickReach = calculateEffectiveReach(AMSALJA_PHYSICAL.legLength, 1.0);
  ...
  describe("calculateMovementSpeed()", () => {
    it("should calculate balanced movement speed for Musa", () => {
      ...
      expect(speed).toBeCloseTo(100, 1); // Normalized baseline
    });
    ...
    it("should properly weight leg length and body weight", () => {
      ...
      expect(calculateMovementSpeed(light)).toBeGreaterThan(calculateMovementSpeed(heavy));
    });
  });
  ...
  describe("calculateDamageModifier()", () => {
    ...
    it("should scale proportionally with muscle mass", () => {
      ...
      expect(calculateDamageModifier(highMuscle)).toBeGreaterThan(calculateDamageModifier(lowMuscle));
    });
  ...
  describe("calculateStaminaRecovery()", () => {
    ...
    it("should penalize older fighters", () => {
      ...
      expect(calculateStaminaRecovery(young)).toBeGreaterThan(calculateStaminaRecovery(old));
    });
    ...
    it("should penalize higher fat mass", () => {
      ...
      expect(calculateStaminaRecovery(lean)).toBeGreaterThan(calculateStaminaRecovery(heavy));
    });
...
describe("Physical Attributes Realism", () => {
  it("should maintain realistic weight ranges", () => {
    ...
    allWeights.forEach(weight => {
      ...
      expect(weight).toBeLessThanOrEqual(95);
    });
  });
  ...
  it("should maintain realistic muscle mass ranges", () => {
    ...
    allMuscleMass.forEach(mass => {
      ...
      expect(mass).toBeLessThanOrEqual(45);
    });
  });
  ...
  it("should maintain realistic body composition", () => {
    ...
    allProfiles.forEach(profile => {
      ...
      // Bone and organs are roughly 20-30kg
...
describe("Korean Martial Arts Biomechanics", () => {
  ...
  it("should support Hapkido joint lock ranges", () => {
    ...
    const musaGrappleRange = calculateEffectiveReach(MUSA_PHYSICAL.armLength, 0.7);
  ...
  it("should reflect traditional warrior balance", () => {
    ...
    // All should be close to baseline (1.0 or 100)
    expect(musaSpeed).toBeGreaterThan(95);
    expect(musaSpeed).toBeLessThan(105);
    expect(musaPower).toBeGreaterThan(0.95);
    expect(musaPower).toBeLessThan(1.15);
    expect(musaDefense).toBeGreaterThan(1.15);
