...
import type { StanceLaterality, StanceWithSide } from "../../trigram/types";
import { parseStanceWithSide } from "../../trigram/types";
import { getDefensiveAnimationsForStance } from "../catalogs/DefensiveAnimations";
import {
  ATTACK_ANIMATIONS_BY_STANCE,
  getAttackAnimationsForStance,
} from "../catalogs/StanceAttackAnimations";
import {
...
} from "../catalogs/StanceLocomotionAnimations";
...
export function getAnimationsForStance(
  ...
  const walk =
    laterality === "right" ? baseWalk : applyLaterality(baseWalk, laterality);
  const run =
    laterality === "right" ? baseRun : applyLaterality(baseRun, laterality);
...
export function getWalkAnimation(
  ...
  return baseAnimation
    ? laterality === "right"
      ? baseAnimation
      : applyLaterality(baseAnimation, laterality)
    : undefined;
}
