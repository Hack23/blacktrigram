...
import type { FootworkPattern } from "./types";
import { AnimationState, FOOTWORK_KOREAN_TERMS } from "./types";
...
describe("Footwork Animation System", () => {
  describe("Animation Configuration", () => {
    describe("Circular Step (원형보)", () => {
      ...
      it("should have same priority as tactical steps", () => {
        const leftPriority =
          ANIMATION_PRIORITY_MAP[AnimationState.FOOTWORK_CIRCULAR_LEFT];
        const stepPriority =
          ANIMATION_PRIORITY_MAP[AnimationState.STEP_FORWARD];
    ...
    describe("Pivot Step (축족회전)", () => {
      ...
      it("should have high priority (non-interruptible)", () => {
        const priority =
          ANIMATION_PRIORITY_MAP[AnimationState.FOOTWORK_PIVOT_LEFT];
    ...
    describe("Slide Step (미끄럼보)", () => {
      ...
      it("should have all 4 directional slide animations", () => {
        const directions: AnimationState[] = [
          AnimationState.FOOTWORK_SLIDE_FORWARD,
      ...
      it("should have lower priority than circular/pivot (interruptible)", () => {
        const slidePriority =
          ANIMATION_PRIORITY_MAP[AnimationState.FOOTWORK_SLIDE_FORWARD];
        const circularPriority =
          ANIMATION_PRIORITY_MAP[AnimationState.FOOTWORK_CIRCULAR_LEFT];
    ...
    describe("Shuffle Step (섞음보)", () => {
      ...
      it("should be fastest footwork pattern", () => {
        const shuffleConfig = DEFAULT_ANIMATION_CONFIGS.get(
          AnimationState.FOOTWORK_SHUFFLE,
        );
        const circularConfig = DEFAULT_ANIMATION_CONFIGS.get(
          AnimationState.FOOTWORK_CIRCULAR_LEFT,
        );
        const pivotConfig = DEFAULT_ANIMATION_CONFIGS.get(
          AnimationState.FOOTWORK_PIVOT_LEFT,
        );
        const slideConfig = DEFAULT_ANIMATION_CONFIGS.get(
          AnimationState.FOOTWORK_SLIDE_FORWARD,
        );

        expect(shuffleConfig?.duration).toBeLessThan(
          circularConfig?.duration ?? 1,
        );
        expect(shuffleConfig?.duration).toBeLessThan(
          pivotConfig?.duration ?? 1,
        );
        expect(shuffleConfig?.duration).toBeLessThan(
          slideConfig?.duration ?? 1,
        );
      });
    });
  });
  ...
  describe("Korean Terminology", () => {
    it("should have Korean terms for all 4 footwork patterns", () => {
      const patterns: FootworkPattern[] = [
        "circular",
        "pivot",
        "slide",
        "shuffle",
  ...
  describe("Animation Timing Requirements", () => {
    it("should meet 60fps performance target", () => {
      const footworkStates: AnimationState[] = [
        AnimationState.FOOTWORK_CIRCULAR_LEFT,
        AnimationState.FOOTWORK_CIRCULAR_RIGHT,
        AnimationState.FOOTWORK_PIVOT_LEFT,
        AnimationState.FOOTWORK_PIVOT_RIGHT,
  ...
  describe("Footwork Pattern Characteristics", () => {
    it("should maintain guard during circular steps", () => {
      ...
      const leftConfig = DEFAULT_ANIMATION_CONFIGS.get(
        AnimationState.FOOTWORK_CIRCULAR_LEFT,
      );
      const rightConfig = DEFAULT_ANIMATION_CONFIGS.get(
        AnimationState.FOOTWORK_CIRCULAR_RIGHT,
      );
    ...
    it("should allow slide steps to be interrupted", () => {
      ...
      const slideStates: AnimationState[] = [
        AnimationState.FOOTWORK_SLIDE_FORWARD,
        AnimationState.FOOTWORK_SLIDE_BACK,
        AnimationState.FOOTWORK_SLIDE_LEFT,
        AnimationState.FOOTWORK_SLIDE_RIGHT,
  ...
  describe("Integration with Existing Systems", () => {
    it("should not conflict with existing step animations", () => {
      const stepStates: AnimationState[] = [
  ...
  describe("Acceptance Criteria Validation", () => {
    ...
    it("✓ AC3: Guard maintained during footwork (non-interruptible)", () => {
      ...
      const circular = DEFAULT_ANIMATION_CONFIGS.get(
        AnimationState.FOOTWORK_CIRCULAR_LEFT,
      );
      const pivot = DEFAULT_ANIMATION_CONFIGS.get(
        AnimationState.FOOTWORK_PIVOT_LEFT,
      );
