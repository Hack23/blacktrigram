...
export function generateRecoveryVisualization(
  ...
  for (let t = 0; t <= animation.duration; t += sampleInterval) {
    ...
    if (t < recoveryStartTime) {
      ...
      t <
      recoveryStartTime + (recoveryEndTime - recoveryStartTime) * 0.6
    ) {
...
export function printRecoveryAnalysis(viz: RecoveryVisualization): void {
  ...
  console.log(
    `  Recovery Duration: ${viz.validation.recoveryDuration.toFixed(1)}ms`
  );
  console.log(`  Recovery Keyframes: ${viz.validation.recoveryKeyframes}`);

  if (viz.validation.issues.length > 0) {
    console.log("\n⚠️  Issues:");
    viz.validation.issues.forEach((issue: string) => {
      console.log(`  - ${issue}`);
    });
  }
  ...
  console.log(
    `  Duration: ${(
      (viz.recoveryEndTime - viz.recoveryStartTime) *
      1000
    ).toFixed(1)}ms`
  );
  ...
  console.log(
    `  Peak: ${peakPoint.tension.toFixed(2)} at ${peakPoint.time.toFixed(3)}s`
  );

  // Find tension at recovery start
  const recoveryStartPoint = viz.timeline.find(
    (p) => Math.abs(p.time - viz.recoveryStartTime) < 0.01
  );
  if (recoveryStartPoint) {
    console.log(
      `  Recovery Start: ${recoveryStartPoint.tension.toFixed(
        2
      )} at ${recoveryStartPoint.time.toFixed(3)}s`
    );
  }

  // Find tension at intermediate recovery
  const intermediateTime =
    viz.recoveryStartTime + (viz.recoveryEndTime - viz.recoveryStartTime) * 0.6;
  const intermediatePoint = viz.timeline.find(
    (p) => Math.abs(p.time - intermediateTime) < 0.01
  );
  if (intermediatePoint) {
    console.log(
      `  Intermediate: ${intermediatePoint.tension.toFixed(
        2
      )} at ${intermediatePoint.time.toFixed(3)}s`
    );
  }
  ...
  console.log(
    `  Final: ${finalPoint.tension.toFixed(2)} at ${finalPoint.time.toFixed(
      3
    )}s`
  );
...
export function generateTensionChart(
  ...
  for (let row = height; row >= 0; row--) {
    ...
    for (const point of sampledPoints) {
      if (Math.abs(point.tension - threshold) < 1 / height) {
        ...
        if (
          point.time >= viz.recoveryStartTime &&
          point.time < viz.recoveryEndTime
        ) {
  ...
  lines.push(
    `    0s${" ".repeat(width - 10)}${viz.timeline[
      viz.timeline.length - 1
    ].time.toFixed(2)}s`
  );
  ...
  lines.push("");
  lines.push(
    `Legend: ${CHART_SYMBOLS.EXECUTION} Execution  ${CHART_SYMBOLS.RECOVERY} Recovery Phase`
  );
...
export function compareRecoveryPhases(
  ...
  for (const [name, animation] of Object.entries(animations)) {
    ...
    const intermediateTension =
      viz.timeline.find(
        (p) =>
          Math.abs(
            p.time -
              (viz.recoveryStartTime +
                (viz.recoveryEndTime - viz.recoveryStartTime) * 0.6)
          ) < 0.01
      )?.tension ?? 0;
    ...
    const line = [
      ...
      `    ${peakTension.toFixed(2)} → ${intermediateTension.toFixed(
        2
      )} → ${finalTension.toFixed(2)}`,
    ].join(" ");
