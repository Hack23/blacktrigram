...
export class MartialArtsAnimationBuilder {
   ...
   * 
   * This method adds TWO keyframes generated by the recovery system:
  ...
  recover(timeOffset: number = 0.2, config?: RecoveryPhaseConfig): this {
    // Build the animation up to current point for recovery calculation
    ...
    // Apply recovery phase system to generate both intermediate and final keyframes
    const withRecovery = addRecoveryPhase(baseAnimation, {
      duration: timeOffset,
      ...config,
    });
    
    // Extract the recovery keyframes (last 2 keyframes added by recovery system)
    const recoveryKeyframes = withRecovery.keyframes.slice(-2);
    
    if (recoveryKeyframes.length !== 2) {
      console.warn("Recovery phase did not generate expected keyframes");
      return this;
    }
    
    // Add both recovery keyframes directly from the recovery system
    // This preserves muscle tension data and breathing synchronization
    this.keyframes.push(recoveryKeyframes[0]); // Intermediate recovery
    this.keyframes.push(recoveryKeyframes[1]); // Final guard position
    
    // Update current time to end of recovery
    this.currentTime = recoveryKeyframes[1].time;
