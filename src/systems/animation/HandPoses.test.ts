...
describe("HandPoses System", () => {
  describe("Hand Pose Definitions", () => {
    it("should define all 7 Korean martial arts hand poses", () => {
      expect(HAND_POSES).toBeDefined();
      expect(Object.keys(HAND_POSES).length).toBe(7);
      ...
      expect(HAND_POSES[HandPoseType.RELAXED]).toBeDefined();
    });
  ...
  describe("Technique Hand Pose Mappings", () => {
    ...
    it("should have appropriate transition durations", () => {
      ...
      expect(TECHNIQUE_HAND_POSES.knife_hand_strike.transitionDuration).toBe(
        0.15
      );
  ...
  describe("Hand Animation State", () => {
    ...
    it("should start transition to new pose", () => {
      const state = createInitialHandAnimationState(HandPoseType.OPEN);
      const updated = updateHandAnimationState(
        state,
        HandPoseType.FIST,
        0.0,
        0.2
    ...
    it("should progress transition over time", () => {
      const state = createInitialHandAnimationState(HandPoseType.OPEN);
      const state1 = updateHandAnimationState(
        state,
        HandPoseType.FIST,
        0.0,
        0.2
      );
      const state2 = updateHandAnimationState(
        state1,
        HandPoseType.FIST,
        0.1,
        0.2
