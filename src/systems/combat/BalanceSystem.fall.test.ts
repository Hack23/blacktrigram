...
function createTestPlayer(balance: number): PlayerState {
  return createMockPlayerState({ balance });
}
...
describe("BalanceSystem - Fall Integration", () => {
  ...
  describe("determineFallType", () => {
    ...
    it("should return side fall from lateral attack", () => {
      const player = createTestPlayer(15);
      const fallTypeLeft = system.determineFallType(
        player,
        -Math.PI / 2,
        "mid",
      );
      const fallTypeRight = system.determineFallType(
        player,
        Math.PI / 2,
        "mid",
      );
  ...
  describe("determineFallTypeFromStance", () => {
    it("should return forward for aggressive stances (Heaven, Fire, Earth)", () => {
      expect(system.determineFallTypeFromStance(TrigramStance.GEON)).toBe(
        "forward",
      );
      expect(system.determineFallTypeFromStance(TrigramStance.LI)).toBe(
        "forward",
      );
      expect(system.determineFallTypeFromStance(TrigramStance.GON)).toBe(
        "forward",
      );
    });

    it("should return backward for defensive stances (Mountain, Water, Thunder)", () => {
      expect(system.determineFallTypeFromStance(TrigramStance.GAN)).toBe(
        "backward",
      );
      expect(system.determineFallTypeFromStance(TrigramStance.GAM)).toBe(
        "backward",
      );
      expect(system.determineFallTypeFromStance(TrigramStance.JIN)).toBe(
        "backward",
      );
    });
    ...
    it("should return backward for Lake stance (fluid retreat)", () => {
      expect(system.determineFallTypeFromStance(TrigramStance.TAE)).toBe(
        "backward",
      );
    });
