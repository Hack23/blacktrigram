...
describe("ArchetypeEnforcer", () => {
  describe("ARCHETYPE_ENFORCEMENT rules", () => {
    it("should define rules for all 5 archetypes", () => {
      ...
      expect(
        ARCHETYPE_ENFORCEMENT[PlayerArchetype.JEONGBO_YOWON],
      ).toBeDefined();
      expect(
        ARCHETYPE_ENFORCEMENT[PlayerArchetype.JOJIK_POKRYEOKBAE],
      ).toBeDefined();
    });

    it("should define signature moves for all archetypes", () => {
      expect(ARCHETYPE_ENFORCEMENT[PlayerArchetype.MUSA].signatureMove).toBe(
        "musa_mountain_breaker",
      );
      expect(ARCHETYPE_ENFORCEMENT[PlayerArchetype.AMSALJA].signatureMove).toBe(
        "amsalja_silent_death",
      );
      expect(ARCHETYPE_ENFORCEMENT[PlayerArchetype.HACKER].signatureMove).toBe(
        "hacker_system_crash",
      );
      expect(
        ARCHETYPE_ENFORCEMENT[PlayerArchetype.JEONGBO_YOWON].signatureMove,
      ).toBe("jeongbo_precision_takedown");
      expect(
        ARCHETYPE_ENFORCEMENT[PlayerArchetype.JOJIK_POKRYEOKBAE].signatureMove,
      ).toBe("jojik_improvised_weapon");
    });
    ...
    it("should define action frequencies for all action types", () => {
      Object.values(PlayerArchetype).forEach((archetype) => {
        ...
        Object.values(AIActionType).forEach((actionType) => {
          expect(
            rules.actionFrequencies[actionType as AIActionType],
          ).toBeDefined();
          expect(
            rules.actionFrequencies[actionType as AIActionType],
  ...
  describe("Musa (Warrior) - Honor Code Enforcement", () => {
    ...
    it("should prohibit feint actions (honor code: no deception)", () => {
      ...
      expect(ARCHETYPE_ENFORCEMENT[archetype].preferredActions).toContain(
        enforced.action,
      );
    ...
    it("should have high attack frequency (70%) and low retreat frequency (2%)", () => {
      ...
      expect(rules.actionFrequencies[AIActionType.ATTACK]).toBe(0.7);
      expect(rules.actionFrequencies[AIActionType.RETREAT]).toBe(0.02);
      expect(rules.actionFrequencies[AIActionType.FEINT]).toBe(0.0); // Prohibited
    });
  });
  ...
  describe("Amsalja (Assassin) - Stealth Tactics", () => {
    ...
    it("should have high technique frequency (60%) and circling (20%)", () => {
      ...
      expect(rules.actionFrequencies[AIActionType.TECHNIQUE]).toBe(0.6);
      expect(rules.actionFrequencies[AIActionType.CIRCLE]).toBe(0.2);
    });
  });
  ...
  describe("Hacker (Cyber Warrior) - Analytical Combat", () => {
    ...
    it("should have high wait frequency (25%) for observation phase", () => {
      ...
      expect(rules.actionFrequencies[AIActionType.TECHNIQUE]).toBe(0.4);
    });
  });
  ...
  describe("Jeongbo Yowon (Intelligence Operative) - Psychological Warfare", () => {
    ...
    it("should have high feint frequency (30%) for psychological pressure", () => {
      ...
      expect(rules.actionFrequencies[AIActionType.FEINT]).toBe(0.3);
      expect(rules.actionFrequencies[AIActionType.TECHNIQUE]).toBe(0.35);
      expect(rules.actionFrequencies[AIActionType.COUNTER]).toBe(0.2);
    });
  });
  ...
  describe("Jojik Pokryeokbae (Organized Crime) - Chaotic Survival", () => {
    ...
    it("should have very high stance change frequency (80%) for unpredictability", () => {
      ...
      expect(rules.actionFrequencies[AIActionType.STANCE_CHANGE]).toBe(0.8);
      expect(rules.actionFrequencies[AIActionType.ATTACK]).toBe(0.5);
    });
  ...
  describe("Utility Functions", () => {
    it("getSignatureMove() should return correct signature move ID", () => {
      expect(getSignatureMove(PlayerArchetype.MUSA)).toBe(
        "musa_mountain_breaker",
      );
      expect(getSignatureMove(PlayerArchetype.AMSALJA)).toBe(
        "amsalja_silent_death",
      );
      expect(getSignatureMove(PlayerArchetype.HACKER)).toBe(
        "hacker_system_crash",
      );
      expect(getSignatureMove(PlayerArchetype.JEONGBO_YOWON)).toBe(
        "jeongbo_precision_takedown",
      );
      expect(getSignatureMove(PlayerArchetype.JOJIK_POKRYEOKBAE)).toBe(
    ...
    it("getActionFrequency() should return correct frequency for action types", () => {
      ...
      expect(
        getActionFrequency(PlayerArchetype.MUSA, AIActionType.ATTACK),
      ).toBe(0.7);
      expect(
        getActionFrequency(PlayerArchetype.MUSA, AIActionType.RETREAT),
      ).toBe(0.02);

      // Amsalja: high technique, high circle
      expect(
        getActionFrequency(PlayerArchetype.AMSALJA, AIActionType.TECHNIQUE),
      ).toBe(0.6);
      expect(
        getActionFrequency(PlayerArchetype.AMSALJA, AIActionType.CIRCLE),
      ).toBe(0.2);
    ...
    it("isActionProhibited() should identify prohibited actions correctly", () => {
      ...
      expect(isActionProhibited(PlayerArchetype.MUSA, AIActionType.FEINT)).toBe(
        true,
      );
      expect(
        isActionProhibited(PlayerArchetype.MUSA, AIActionType.ATTACK),
      ).toBe(false);

      // Amsalja has no prohibitions
      expect(
        isActionProhibited(PlayerArchetype.AMSALJA, AIActionType.FEINT),
      ).toBe(false);
      expect(
        isActionProhibited(PlayerArchetype.AMSALJA, AIActionType.RETREAT),
      ).toBe(false);

      // Hacker has no prohibitions
      expect(
        isActionProhibited(PlayerArchetype.HACKER, AIActionType.WAIT),
      ).toBe(false);
    });
  });
  ...
  describe("Archetype Distinctiveness", () => {
    ...
    it("each archetype should have at least one action with >30% frequency", () => {
      ...
      archetypes.forEach((archetype) => {
        ...
        const highFrequencies = frequencies.filter((freq) => freq >= 0.3);
        expect(highFrequencies.length).toBeGreaterThan(0);
      });
    });

    it("Musa should be the ONLY archetype with prohibited FEINT", () => {
      const archetypesWithFeintProhibition = Object.values(
        PlayerArchetype,
      ).filter((archetype) =>
        ARCHETYPE_ENFORCEMENT[
          archetype as PlayerArchetype
        ].prohibitedActions.includes(AIActionType.FEINT),
      );
    ...
    it("Jojik should have the HIGHEST stance change frequency", () => {
      ...
      expect(stanceChangeFrequencies[0].archetype).toBe(
        PlayerArchetype.JOJIK_POKRYEOKBAE,
      );
      expect(stanceChangeFrequencies[0].frequency).toBe(0.8);
    });
