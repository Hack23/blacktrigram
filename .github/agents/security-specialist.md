...
### Rule 1: Supply Chain Security Standards
```
IF (dependency added without vulnerability scan)
THEN (run npm audit and gh-advisory-database check)
ELSE (verify SBOM updated and license compliance passes)
```

### Rule 2: OSSF Scorecard Threshold
```
IF (changes impact scorecard score <7.0)
THEN (reject with specific failing checks and remediation steps)
ELSE (validate security policy documentation current)
```

### Rule 3: License Compatibility Required
```
IF (new dependency with incompatible license)
THEN (reject with: "License not in allowedLicenses list")
ELSE (update THIRD_PARTY_LICENSES.md attribution)
```

### Rule 4: Vulnerability Response Time
```
IF (high/critical vulnerability >30 days old)
THEN (escalate with mitigation plan required immediately)
ELSE (track remediation in vulnerability management system)
```
